...
<html>
	...
	<body>
		<s:form namespace="/sysBusiness" method="post">
			<s:hidden name="sysBusinessVO.id" />
			...
			<table align="center" class="tableBorder"
				...
				<tr class="forumRowHighlight"
					...
					<td colspan="4">
						<s:if test="edit">
							客户编号(<s:property value="sysCustVO.id" />)订单修改:订单编号(<s:property
								value="sysBusinessVO.id" />)
						</s:if>
						<s:else>
							客户编号(<s:property value="sysCustVO.id" />)增加订单
						</s:else>
				...
				<tr>
					...
					<td>
						<s:textfield name="sysBusinessVO.sysCustId" data="客户编号"
							disabled="true" />
						<s:hidden name="sysBusinessVO.sysCustId" />
					</td>
					<td>
						合同编号:*
					</td>
				...
				<tr>
					...
					<td>
						<s:select list="basePaymentVOs" listKey="id"
							...
							name="sysBusinessVO.basePayment.id" />
					</td>
					<td>
						交付方式:
					</td>
					<td>
						<s:select list="baseDeliveryVOs" listKey="id"
							listValue="deliveryName" name="sysBusinessVO.baseDelivery.id" />
					</td>
				</tr>
				<tr>
					<td>
							...
							<tr>
								<td>
									<div align="center">
										<s:if test="edit">
											<s:submit value="修  改"
												action="sysBusinessForCustAction!update"
												onclick="return submitCheck(this) && isNumber(this);"></s:submit>
										</s:if>
										<s:else>
											<s:submit value="增  加" action="sysBusinessForCustAction!save"
												onclick="return submitCheck(this) && isNumber(this);"></s:submit>
										</s:else>
										&nbsp;&nbsp;
